...
<body class="bg-mesh h-screen flex items-center justify-center p-6 overflow-hidden">
    <div class="w-full max-w-md">
        ...
        <div class="flex flex-col items-center mb-6">
            ...
            <p class="text-gray-600 dark:text-slate-300 font-medium mt-2">Sign in to manage your workspace</p>
        </div>

        <div class="glass-card rounded-2xl p-8 shadow-xl shadow-slate-200/60 dark:shadow-none">
            @if(!empty($maintenance['enabled']))
                <div class="mb-6 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400">
                    ...
                    <p class="text-xs font-medium opacity-90 leading-relaxed">
                        {{ $maintenance['message'] ?? 'We are currently performing scheduled maintenance and updates.' }}
                    </p>
                    <div class="mt-2 text-[11px] font-semibold opacity-75 border-t border-amber-500/20 pt-2">
                        Only Administrators can log in during maintenance.
                    </div>
                </div>
            @endif
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-6">
                @csrf
                <div>
                    <label class="block text-xs font-bold text-gray-700 dark:text-slate-300 mb-2 ml-1">Email or Username</label>
                ...
                <div x-data="{ show: false }">
                    <div class="flex justify-between items-center mb-2 ml-1">
                        <label class="text-xs font-bold text-gray-700 dark:text-slate-300">Password</label>
                    </div>
                    <div class="relative">
                ...
                <div class="flex items-center ml-1">
                    <input type="checkbox" name="remember" id="remember" class="w-4 h-4 text-blue-600 border-gray-300 dark:border-slate-700 rounded focus:ring-blue-500 bg-white dark:bg-slate-800">
                    <label for="remember" class="ml-3 text-sm font-medium text-gray-700 dark:text-slate-300">Stay signed in</label>
                </div>
