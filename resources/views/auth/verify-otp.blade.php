...
<body class="bg-mesh h-screen flex items-center justify-center p-6 overflow-hidden">
    <div class="w-full max-w-md">
        ...
        <div class="flex flex-col items-center mb-6">
            ...
            <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Enter Verification Code</h1>
            <p class="text-gray-600 dark:text-slate-300 font-medium mt-2 text-center text-sm">We sent a 6-digit OTP code to <span class="font-bold text-gray-900 dark:text-white">{{ $email }}</span></p>
        </div>
        ...
        <div class="glass-card rounded-2xl p-8 shadow-xl shadow-slate-200/60 dark:shadow-none">
            ...
            <form action="{{ route('password.otp.verify') }}" method="POST" class="space-y-6">
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-700 dark:text-slate-300 mb-2 ml-1">6-Digit OTP Code</label>
                    <input type="text" name="otp" required autofocus maxlength="6" pattern="[0-9]{6}" inputmode="numeric"
                        class="w-full px-4 py-3 text-center tracking-[0.5em] text-xl font-bold bg-white dark:bg-slate-800/50 border border-gray-300 dark:border-slate-700 rounded-xl text-gray-900 dark:text-white focus:ring-4 focus:ring-blue-500/20 focus:border-blue-600 transition-all outline-none placeholder:text-gray-400 dark:placeholder:text-slate-500"
                        placeholder="••••••">
                    @error('otp')
            ...
            <form action="{{ route('password.email') }}" method="POST" class="mt-4 text-center">
                @csrf
                <input type="hidden" name="email" value="{{ $email }}">
                <button type="submit" class="text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline bg-transparent border-0 cursor-pointer">
                    Didn't receive the code? Resend OTP
                </button>
            </form>

            <div class="text-center mt-4 border-t border-gray-200 dark:border-slate-800 pt-4">
                <a href="{{ route('password.request') }}" class="text-xs font-semibold text-gray-500 dark:text-slate-400 hover:underline">
